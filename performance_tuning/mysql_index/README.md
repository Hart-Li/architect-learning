...
1. InnoDB 底层存储结构及其定义（B+树：叶子节点存储数据、叶子节点从左往右是有序的、叶子节点上是一个循环链表）
2. 为什么使用 B+ 树，而不使用B树或 Hash 索引（范围查询、哈希冲突、减少 I/O，节省存储空间）
3. 为什么一般建议自己建一个整型主键，而且尽量使用自增性质的主键（自己不建主键MySQL优先选取唯一键其次使用隐藏的唯一ID、整型比较比字符类型比较要快、自增有助于快速建立索引，防止反复平衡索引）
4. B+ 树三层能存储多少数据？为什么？（2kw左右，一页16K，节点由节点数据和下一个的指针组成，节点数据假设为 bigint 类型，则为 8 字节，指针 6 字节，因此一页存储大约1170个数据，第一层和第二层一页可以存储的数据量都是 1170，第三层需要存储具体的数据，假设一个节点为1k，则数据量为16，所有总数为 1170*1170*16）
5. 为什么非主键索引结构的叶子节点存储的是主键值？（一致性和节省存储空间，一致性是指若是所有的索引都存储数据的话，则需要所有的索引对应的叶子节点的数据需要保持完全一致，修改或新增成本都很高）
6. 什么是聚集索引？（叶子节点中存储了全部数据的索引）